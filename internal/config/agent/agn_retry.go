...
type Sender func(context.Context, agnTypes.OptionsAgent, chan storage.MemStorage) error
...
func Retry(sender Sender, retries int, delay time.Duration) Sender {
	return func(ctx context.Context, cfg agnTypes.OptionsAgent, metricsCh chan storage.MemStorage) error {
		for r := 0; ; r++ {
