...
type Sender func(context.Context, agn_types.OptionsAgent, chan storage.MemStorage) error
...
func Retry(sender Sender, retries int, delay time.Duration) Sender {
	return func(ctx context.Context, cfg agn_types.OptionsAgent, metricsCh chan storage.MemStorage) error {
		for r := 0; ; r++ {
