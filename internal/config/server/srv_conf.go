...
func parseOptions() (types.Options, error) {
	...
	flag.StringVar(&cfg.CryptoKey, "crypto-key", "", "Path to private key file") // Исправлено
	...
	// Если указан путь к приватному ключу, загружаем его в `crypto.PrivateKey`
	keyPath := cfg.CryptoKey
	if keyPath == "" {
		keyPath = "private.pem" // Новый путь по умолчанию
	}

	log.Printf("Загрузка приватного ключа из %s...", keyPath)
	crypto.PrivateKey, err = crypto.LoadPrivateKey(keyPath)
	if err != nil {
		log.Fatalf("Ошибка загрузки приватного ключа: %v", err)
	}
	log.Println("Приватный ключ успешно загружен, расшифровка включена")
