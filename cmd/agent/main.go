...
func (a *Agent) pushMetricsToServer(name string, metric metrics.Metric) {
	// data := fmt.Sprintf("%v", metric.Value)
	// url := fmt.Sprintf("http://localhost:8080/update/%s/%s/%s", metric.Type, name, data)
	// log.Printf("Формирование URL для отправки %s", url)
	// // log.Printf("Сбор метрики: %s", data)
